...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Samurai\n' // Nombre
            + 'ORG:Samurai;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=573175852207:+57 317 585 2207\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'Conectado by Samurai')
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Joder @${num.split('@')[0]}\Que bueno verte por aca seas Bienvenido a *${mdata.subject}*\n\nEspero la pases super\n\nPara utilizar el bot registrate con el comando ${prefix}daftar y tu nombre\n\nPara ver los demas comandos utiliza ${prefix}samurai\n\nOjito con el spam 🧐\n\nby Samurai`
				...
				teks = `Hasta que te fuiste😎 @${num.split('@')[0]}👋\n\nDa igual ya no te queríamos😎`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[❗] Este comando es solo para admins facheros 😎! ❌',
					ownerB: '[❗] Este comando solo lo usa Samurái Bot! ❌',
					admin: '[❗] Este comando es solo para admins facheros 😎! ❌',
					Badmin: '[❗] Este comando solo se puede usar cuando el bot se convierte en administrador! ❌',
                                        daftarB: `「OJITO」\n\n !\n\nNo estas registrado en mi base de datos 😳 \n\nComando : ${prefix}daftar Nombre\nEjemplo : ${prefix}daftar Samurai`,
				}
			}
			...
			const ownerNumber = ["573175852207@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '573175852207@s.whatsapp.net'
                        /******ApiKey Input******/
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					teks = `❏ *Título* : ${anu.title}\n❏ *Tamaño del archivo* : ${anu.result.size}\n\nAwanta wee ya estoy enviando el archivo 😎😇`
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionar A Todos 〙✪══\n╠➥'+teks+'╚═〘 Samurai-Bot 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionar A Todos 〙✪══\n╠➥'+teks+'╚═〘 Samurai-bot 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Mencionar A Todos 〙✪══\n╠➥'+teks+'╚═〘 Samurái-bot 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
			...
			case 'cerrar':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'By Samurái'})
						fs.unlinkSync(ran)
					})
					break
                	case 'tomp3':
            ...
            break
				...
				if (!isUser) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/tiktok?apikey=apivinz&url=${args[0]}&apikey=apivinz`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
